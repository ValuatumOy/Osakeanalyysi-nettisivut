...
// every role is three numbers (Esa, 17.8.2026):
//
...
//
// All three are demand-tuned, so they must be changeable without a code change:
...
const DEFAULT_LIMITS = {
  ...
  analyst: { generations: 1, basePicks: 10, analystReads: 20 },
  reader: { generations: 1, basePicks: 5, analystReads: 10 },
  ...
  none: { generations: 0, basePicks: 0, analystReads: 0 },
  investor: { generations: 0, basePicks: 3, analystReads: 10 },
  'investor:year': { generations: 0, basePicks: 5, analystReads: 10 },
  investor_plus: { generations: 1, basePicks: 10, analystReads: 20 },
  'investor_plus:year': { generations: 1, basePicks: 15, analystReads: 20 },
  coverage: { generations: 0, basePicks: 0, analystReads: 0 }, // its own yearly path
};

const ZERO = { generations: 0, basePicks: 0, analystReads: 0 };
...
// each of the three numbers — an upgrade must never take something away.
